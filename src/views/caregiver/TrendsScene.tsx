 ...
 */
function regress(values: readonly number[]): {
  slope: number;
  intercept: number;
  stddev: number;
} {
    ...
    <div className="space-y-5">
      <header>
        <h1 className="font-display text-3xl font-semibold leading-tight text-slate-900 sm:text-4xl">
          {t("trendsScene.cognitiveTrends")}
        </h1>
        <p className="mt-2 max-w-md text-sm leading-6 text-slate-600">
          {t("trendsScene.howThePatientSMemorySpanAndReact")}
        </p>
      </header>
      ...
      {signals.length > 0 ? (
        ...
        >
            ...
            {declining.length > 0 ? (
              <>
                ...
                <span className="text-amber-900">
                  {t("trendsScene.declineSignalAcrossTheLast")} {signals[0]!.windowSize} sessions
                </span>
              </>
            ) : (
              <>
                ...
                <span className="text-emerald-900">
                  {t("trendsScene.steadyAcrossTheLast")} {signals[0]!.windowSize} sessions
                </span>
              </>
            )}
          </div>
          <ul className="mt-3 grid gap-2 sm:grid-cols-2">
            {signals.map((s) => (
              <li key={s.label} className="rounded-xl bg-white/70 p-3 ring-1 ring-slate-200">
        ...
        <section className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-5 text-sm text-slate-600">
          {t("trendsScene.aTrendSignalNeedsAtLeastFourFina")}
        </section>
      )}
      ...
      <section className="rounded-2xl border border-slate-200 bg-slate-50 p-5 text-sm leading-6 text-slate-700">
        <p>
          <strong>{t("trendsScene.howDeclineDetectionWorks")}</strong>{" "}
          {t("trendsScene.weFitALeastSquaresLineThroughThe")} {SLOPE_WINDOW}{" "}
          {t("trendsScene.finalizedSessionsASlopeBeyondATh")}
        </p>
      </section>
    </div>
  );
