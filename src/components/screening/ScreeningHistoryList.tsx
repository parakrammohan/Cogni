...
export function useInvalidateScreeningHistory() {
  ...
  return (patientId: string | null, model: ModelKey) => {
    if (!patientId) return;
    qc.invalidateQueries({ queryKey: screeningHistoryKeys.list(patientId, model) });
  };
}
    ...
    <section className="rounded-2xl border border-slate-200 bg-white shadow-(--shadow-soft)">
      <header className="flex items-center gap-2 border-b border-slate-100 px-4 py-3">
        ...
        <span className="text-xs font-semibold uppercase tracking-wider text-slate-500">
          Past runs on this patient
        </span>
        {data && data.length > 0 ? (
          <span className="ml-auto text-xs text-slate-500">{data.length} record{data.length === 1 ? "" : "s"}</span>
        ) : null}
      </header>
      ...
      {isLoading ? (
        ...
        <p className="px-4 py-6 text-sm text-red-700">
          Could not load past runs. (The backend may be cold-starting.)
        </p>
      ) : !data || data.length === 0 ? (
        <p className="px-4 py-6 text-sm text-slate-500">
          No past runs yet. Submit the form above to record one.
        </p>
      ) : (
        <ul>
          {data.map((row) => {
...
function InputsPreview({ inputs }: { inputs: Record<string, unknown> }) {
  ...
  return (
    <div className="bg-slate-50 px-4 pb-4 pl-10 text-xs leading-5 text-slate-700">
      <p className="mb-1 font-semibold text-slate-600">Inputs at the time of this run</p>
