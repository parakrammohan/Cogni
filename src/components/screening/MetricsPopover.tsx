...
const BINARY_METRICS: MetricDef[] = [
  ...
  },
  {
    ...
    format: PCT,
    explainer:
      "When the model says someone is at risk, how often it's actually right.",
  },
  {
    key: "cv_recall",
    label: "Recall",
    format: PCT,
    explainer:
      "Of the people who really are at risk, how many the model correctly flags.",
  },
  {
    key: "cv_f1",
    label: "F1 score",
    format: RAW,
    explainer:
      "A single number that balances precision and recall — closer to 1.00 is better.",
  },
  {
    key: "cv_auc",
    label: "AUC",
...
export function MetricsPopover({
  ...
  if (!meta) return null;
  const definitions = meta.task === "multiclass_classification" ? MULTICLASS_METRICS : BINARY_METRICS;
  const available = definitions.filter(
    (d) => typeof meta.metrics?.[d.key] === "number",
  );
  const trainSize =
      ...
      {open ? (
        ...
        >
          <div className="mb-3 flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm leading-6 text-slate-600">
                Numbers below come from holding out a chunk of the
                training data, predicting on it, and comparing against
                the truth. A guide to how the model behaves on unseen
                records — not a clinical guarantee.
              </p>
            </div>
            <button
              type="button"
          ...
          <ul className="space-y-2.5">
            {available.map((d) => {
              ...
              return (
                <li
                  key={d.key}
                  className="flex flex-col gap-0.5 rounded-xl bg-slate-50 px-3 py-2"
                >
          ...
          <div className="mt-3 grid gap-1 border-t border-slate-100 pt-3 text-xs leading-5 text-slate-500">
            <div>
              <span className="font-semibold text-slate-700">Model type:</span>{" "}
              {meta.model_type}
            </div>
            <div>
            ...
            {trainSize !== null ? (
              <div>
                <span className="font-semibold text-slate-700">Training rows:</span>{" "}
