...
function generateBubbles(): Bubble[] {
  ...
  while (placed.length < GOAL) {
    ...
    const tooClose = placed.some(
      (b) => Math.hypot(b.x - x, b.y - y) < MIN_DIST,
    );
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[0.85fr_1.15fr]">
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-(--shadow-soft)">
        ...
        <h3 className="mt-2 font-display text-2xl text-slate-900">Bubble pop</h3>
        <p className="mt-2 text-sm leading-6 text-slate-600">
          Pop the bubbles in order from <strong>1</strong> to <strong>{GOAL}</strong>. Tap the
          wrong one and it shakes — you&apos;ll need to keep going from where you were.
        </p>
        <div className="mt-4 flex flex-wrap gap-2">
          <Badge tone="info">Next: {completed ? "✓" : next}</Badge>
          <Badge tone={mistakes ? "warning" : "calm"}>{mistakes} mistakes</Badge>
          <Badge tone="info">{(elapsedMs / 1000).toFixed(1)}s</Badge>
          {completed ? <Badge tone="good">Cleared!</Badge> : null}
        </div>
        <div className="mt-5">
        ...
        {completed ? (
          <div className="mt-4 flex items-start gap-3 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-900">
            ...
            <div>
              All {GOAL} popped in <strong>{(elapsedMs / 1000).toFixed(1)} s</strong> with
              {" "}
